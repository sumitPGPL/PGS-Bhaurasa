...
const LifeAtSchoolMonument = () => {
    ...
    return (
        <div className="w-11/12 mx-auto mt-8 p-4 bg-white rounded-lg shadow-md">
            <div className="flex justify-between overflow-scroll">
                {Object.keys(monumentsData).map((monument) => (
                    <button
                        ...
                        className={`p-2 text-sm focus:outline-none ${selectedMonument === monument ? 'rounded-t-lg font-bold bg-bblue p-5' : 'font-normal'}`}
                    >
            ...
            <div >
                <p className="text-gray-700 bg-bblue p-5 rounded-b-lg">{monumentsData[selectedMonument].content}</p>
            </div>
        </div>
    );
};
