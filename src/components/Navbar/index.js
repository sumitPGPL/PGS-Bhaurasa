          ...
          <div className="bg-black text-white">
            <div className="flex items-center justify-between p-4">
             ...
             <a>{svgIcon}</a> */}
              ...
              {/* Login button */}
              <div className="flex">
                <Link className="text-xs px-4 py-2 leading-none rounded-full hover:bg-blue-300 hover:text-black border-2" href='/admin/login' onClick={handleStuffLogin}>
                    ...
                    {dropdownItems.map(({ name, values }, index) => (
                      <div key={name}>
                        ...
                        {activeDropdown === index && values.length > 0 && (
                          <ul className="shadow-xl pl-5 rounded-lg">
                            {values.map((value, index) => (
                              <li key={index} className="py-2">
                                {/* Use handleSidebarItemClick to navigate and close the sidebar */}
                                <div onClick={() => handleSidebarItemClick(value.path)}>
                                  {value.displayName}
                                </div>
                              </li>
                            ))}
...
let dropdownItems = [
  {
    ...
    values: [
      ...
      { displayName: 'Chairman message', path: '/about/chairmans-message' },
      { displayName: 'Director msg', path: '/about/directors-message' },
      { displayName: 'Principle msg', path: '/about/principals-message' },
  ...
  },
  {
    name: 'NEWS',
    values: [
      { displayName: 'News and event', path: '/news' },
